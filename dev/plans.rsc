...
<tr>
<td>session-dispatch skill の v-next（器 15、中身 17、7 論点）</td><td>一部実装（文書は凍結、器の一部は別の依頼で実装）</td><td>9 月上旬</td><td><code>4d01edf2</code></td>
</tr>
...
<tr><td>ランチャーの作り直し（<code>2026-09-03-launcher-redesign-requirements.md</code>）</td><td><span class="chip ok">稼働</span>（Phase 1〜3。TUI の撤去は未判断）</td><td>9 月上旬</td><td><code>67499d4b</code></td>
</tr>
...
<tr><td>公開アプリの一本化（改名を含む）</td><td>一部実装（公開の書き出し、壁紙、ペット既定オフまで。改名は未）</td><td>9 月中旬</td><td><code>746c4961</code></td>
</tr>
<tr>
<td>Web ペイン（<code>2026-08-27-web-pane-chatgpt-requirements.md</code>）</td><td><span class="chip ok">稼働</span>（計画 1 件から、実装 6 サービス）</td><td>9 月上旬</td><td><code>8beaade9</code></td>
</tr>
<tr>
<td>Web の自動操作層（<code>2026-09-08-web-automation-layer.md</code>）</td><td><span class="chip ok">稼働</span>（Mac の一部は未対応）</td><td>9 月上旬</td><td><code>29747424</code></td>
</tr>
<tr><td>窓の切り離し（<code>2026-08-09-multiwindow-tearout-design.md</code>）</td><td><span class="chip ok">稼働</span></td><td>8 月上旬</td><td><code>703f15ee</code></td>
</tr>
<tr><td>ペイン再配置の群（8 月下旬の 11 本）</td><td><span class="chip ok">稼働</span>（Grok 版と監査記録 4 本は置き換え）</td><td>8 月末</td><td><code>774daa99</code></td>
</tr>
<tr>
<td>Jev の製品化（9 月下旬の 3 本）</td><td><span class="chip ok">稼働</span>（本番は 9 月下旬から有効）</td><td>9 月下旬</td><td><code>d186c987</code></td>
</tr>
<tr>
<td>hook の契約と線の仕様（8 月末）</td><td><span class="chip ok">稼働</span>（helper は Rust の予定が Python に）</td><td>8 月末</td><td><code>b9b08df9</code></td>
</tr>
<tr><td>質問カードの契約（8 月末）</td><td>一部実装（hook は質問の中身を運ばない）</td><td>8 月末</td><td><code>0a0bebb6</code></td>
</tr>
<tr>
<td>orca からの取り込み（8 月下旬）</td><td>一部実装（OS の通知と、hook の健全性表示は未）</td><td>8 月下旬</td><td><code>b9b08df9</code></td>
</tr>
<tr>
<td>data.json の版の守り（8 月下旬）</td><td><span class="chip ok">稼働</span></td><td>8 月下旬</td><td><code>1de52fe5</code></td>
</tr>
<tr><td>復元の取り違えの修正（8 月下旬）</td><td><span class="chip ok">稼働</span></td><td>8 月下旬</td><td><code>000a86c</code></td>
</tr>
<tr><td>ペットの状態（9 月上旬）</td><td><span class="chip ok">稼働</span></td><td>9 月上旬</td><td><code>a0e847ac</code></td>
</tr>
<tr><td>ダッシュボードの計器行（8 月下旬）</td><td>一部実装（文書は「未着手」のまま）</td><td>8 月下旬</td><td><code>024579d5</code></td>
</tr>
<tr><td>要対応の帯（9 月上旬）</td><td>破棄（同じ週に「いらない」。ブランチにだけある）</td><td>9 月上旬</td><td><code>e3fec047</code></td>
</tr>
...
<tr>
<td><code>docs/current-state.md</code></td><td><span class="chip warn">古い</span>（現行版を 9 月上旬の版と書く）</td></tr>
<tr>
<td>ADR 0002（スマホに端末を載せる）・0008（Web ペインは CDP ポート付き）</td><td>実態と違うまま accepted</td>
</tr>
<tr>
<td>ADR 0012（hook 由来の注意の印）</td><td>実装済みなのに「提案」のまま</td></tr>
<tr>
<td><code>README.md</code> の報告インボックス・ペイン再配置の記述</td><td>削除済み、有効化済みの機能を、古い状態で書く</td></tr></tbody></table>13:["$","section","freshness",{"id":"freshness","children":[["$","h2",null,{"children":"文書の鮮度"}],[["$","div","0",{"dangerouslySetInnerHTML":{"__html":"$19"}}]]]}]
14:["$","section","doc-only",{"id":"doc-only","children":[["$","h2",null,{"children":"文書にあって実装に無いもの"}],[["$","div","0",{"dangerouslySetInnerHTML":{"__html":"<ul><li><strong>エージェントの家</strong>の全部（<code>houses.db</code>、house CLI、写しの正本、読む範囲の契約、名指しの問い、親の道具、横断の表示）と、忘却と容量の要件</li><li><strong>観測契約</strong>の重複の裁定、hook の実機計測、メール、返信案の置き場所、WorkOrder の段 2</li><li><strong>ランチャー</strong>の TUI の撤去の判断（10 月上旬ごろの予定）と、browser と online が PTY を持たない扱い</li><li><strong>共有ワークスペース</strong>の工程 2 以降と、master への合流</li><li><strong>1.0 計画</strong>の段 1 の残りと、段 2〜4</li> <li><strong>store の境界、端末ランタイムの分割、型付きエラー、PTY デーモン</strong></li><li><strong>orca からの取り込み</strong>の OS の通知と、hook の健全性の表示</li><li><strong>hook の契約</strong>の Rust 製 helper と、診断の画面</li></ul>"}}]]]}]
15:["$","section","code-only",{"id":"code-only","children":[["$","h2",null,{"children":"実装にあって文書に無いもの"}],[["$","div","0",{"dangerouslySetInnerHTML":{"__html":"<ul><li>ダッシュボードの気づき列の作り替え（9 月中旬。報告インボックスの削除と、session-board の取り込みの停止）</li><li>委譲の見張り（9 月上旬に導入、9 月中旬に停止）。設計文書は <code>docs/plans/</code> に無く、記録はハーネス側の報告 HTML だけ</li><li>/seats（ハーネス側の横断の表示）</li><li>ランチャーの Phase 3（9 月上旬）</li><li>委譲便の器の一部（ワークスペースの作成と削除、裏の PTY、model と effort。最後のものは、素の起動のときだけ効く）。ワークスペースを閉じる規則（前面を拒否）は、7 論点の案と別のものを採った</li><li>内蔵 remote の削除（共有ワークスペースの設計の土台を消した）</li><li>起動の一時環境変数の、2 本目の一覧（<code>src/lib/spawnLaunchEnv.ts</code>）</li><li>起動できる相手の広がり（追跡する 4 種に対して、起動は 7 種）</li><li>9 月中旬の「新機能は止める」の後にも、機能が足されたこと</li><li>ポケット mycmux が、要件を超えていること（ペアリング、メールの下書きと送信の API、ペインの起動と送信）</li></ul>"}}]]]}]
16:["$","section","unverified",{"id":"unverified","children":[["$","h2",null,{"children":"未確認事項"}],[["$","div","0",{"dangerouslySetInnerHTML":{"__html":"<ol><li>家の 1 往復試験の方針への回答（チャットだけで答えた可能性）</li><li>9 月上旬以降に、チャットだけで下された裁定</li><li>要対応の帯の「いらない」と、ペインバー 2 段化の保留の原文（開発の記録の伝聞）</li><li>共有ワークスペースのブランチを合流したときの、衝突の範囲</li><li>切り離した窓を再起動後に戻す動きが、Windows でも効くか</li><li>報告インボックスの要約が、画面の削除後も働いているか</li><li>8 月末の再起動でワークスペースが減った件と、9 月中旬の復元の修正が、同じ事象か</li><li>復元の修正と、ペットの本番での確認が行われたか</li><li>本番プロファイルで Jev が有効か（周辺の仕組みの調査では、有効と確認）</li><li>Web ペインの要件の一部が、別の方式で代わっているか</li><li>ポケットのメール送信 API を認めた裁定</li><li>旧 remote のポート番号（7682 と 7683 の 2 表記）</li><li>hook の版をまたいだ試験の有無と、Grok の hook が届くか</li><li><code>docs/agent-integration.md</code> と README の付録の全数照合</li><li>ビルドとテストは、誰も実行していない（静的な読解と git 履歴による）</li></ol>"}}]]]}]
17:["$","nav",null,{"className":"next-page","aria-label":"次に読む","children":[["$","span",null,{"className":"next-label","children":"次に読む"}],["$","a",null,{"href":"/dev/responsibility","children":"責務の分け方"}],"$undefined"]}]
